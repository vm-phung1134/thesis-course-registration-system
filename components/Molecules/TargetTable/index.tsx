...
export const TargetTable: FC<ITargetTableProps> = ({ namePhase, children }) => {
  return (
    <div className="p-10 shadow-2xl flex flex-col gap-3 justify-center items-center">
      <div className="border-2 rounded-full border-green-700">{children}</div>
      <p className="uppercase text-green-700 font-medium">{namePhase}</p>
      <p className="text-sm italic">Week 1 - Week 3</p>
      <p>The goal needs to be achieved at this phase</p>
      ...
      <Button className="w-full mt-5 bg-green-700 text-gray-200" title="Get back report progress" />
    </div>
  );
};
